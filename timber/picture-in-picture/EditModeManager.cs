...
public class EditModeManager : Camera2D
{
    ...
    private void UpdateScaleAndPosition()
    {
        ...
        GD.Print("edit mode: " + edit_mode + " tex size : " + tex_size + " size required : " + scaled_size + " window size : " + windowSize);
    ...
    // {
    //     // Get global mouse pos and convert to local pos
    //     Vector2 globalMousePos = GetViewport().GetMousePosition();
    //     Vector2 localMousePos = computer_screen.GetGlobalTransform().AffineInverse().Xform(globalMousePos);

    //     // Scale mouse pos
    //     Vector2 scaledMousePos = localMousePos / (computer_screen.Scale * current_zoom_level);
